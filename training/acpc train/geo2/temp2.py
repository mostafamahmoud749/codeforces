...
def get_perpendicular_point_2d_complex(line_point, line_dir, distance, side=1):
    A = line_point     
    d = line_dir        
    
    # Multiplying by 1j rotates the direction vector 90 degrees
    perp = d * 1j
    
    # Normalize (abs gives magnitude) and scale by distance
    unit_perp = perp / abs(perp)
    
    # Offset point
    P = A + side * distance * unit_perp
    return P
...
def get_polygon_diameter(n, s):
    """
    Calculates the maximum diameter of a regular polygon.
    - If n is even: diameter is the vertex-to-vertex distance (2 * circumradius).
    - If n is odd: diameter is the vertex-to-opposite-side distance (circumradius + apothem).
    """
    if n % 2 == 0:
        return s / math.sin(math.pi / n)
    else:
        return (s / 2.0) * (1.0 / math.tan(math.pi / (2 * n)))

def count_segment_lattice_points(p1, p2):
    """
    Counts total integer points on the segment from p1 to p2 (including both endpoints).
    p1, p2: complex numbers representing integer coordinates (e.g., 2 + 3j)
    """
    dx = abs(int(round(p1.real)) - int(round(p2.real)))
    dy = abs(int(round(p1.imag)) - int(round(p2.imag)))
    return math.gcd(dx, dy) + 1

def polygon_boundary_lattice_points(pts):
    """
    Calculates the total number of lattice points on the boundary (edges) of a polygon.
    pts: list of complex numbers representing the polygon vertices in order.
    """
    n = len(pts)
    b_total = 0
    ...
    for i in range(n):
        p1 = pts[i]
        p2 = pts[(i + 1) % n]  # Next vertex, wrapping around to start
        
        dx = abs(int(round(p1.real)) - int(round(p2.real)))
        dy = abs(int(round(p1.imag)) - int(round(p2.imag)))
        
        # We add math.gcd(dx, dy) instead of +1 to avoid double-counting shared vertices
        b_total += math.gcd(dx, dy)
        
    return b_total

def get_polygon_area(n, s):
    """
    Calculates the area of a regular polygon given the number of 
    vertices (n) and the side length (s).
    """
